...
impl RenderArea {
    ...
    fn iter(&self) -> RenderAreaIter {
        RenderAreaIter {
            min: self.min,
            max: [self.min[0] + self.size[0], self.min[1] + self.size[1], self.min[2] + self.size[2]],
...
impl WorldRender {
    ...
    pub fn draw<S: glium::Surface>(
        ...
        let chunk_iter = self.render_chunks
            .iter()
            .filter_map(|opt| {
                opt.as_ref().and_then(|&(pos, ref chunk)| {
                    let corners: Vec<[f32; 3]> = CORNER_OFFSET
                        ...
                        .collect();
                    let visible = corners.iter().any(|c| c[2] < 1.) && corners.iter().any(|c| c[0] < 1.) &&
                ...
                if let Some(update) = update {
                    if update.1.is_empty() {
                        ...
                        match &mut self.render_chunks[index] {
                            ...
                            none => {
                                *none = Some((update.0, RenderChunk::from_update(facade, update.1)));
                            }
                        }
                    }
                } else {
...
impl RenderChunkUpdater {
    fn new(
        area: Arc<Mutex<RenderArea>>,
        world: Arc<World>,
        update_sender: Sender<UpdateMessage>) -> Self {
        RenderChunkUpdater {
            shared_area: area,
        ...
        loop {
            ...
            if let Some(pos) = world.poll_chunk_update() {
                if let Some(&index) = self.chunk_indices.get(&pos) {
                    ...
                    self.update_sender.send((index, Some((pos, update)))).unwrap();
                }
            } else {
    ...
    fn update_area(&mut self, new_area: RenderArea) {
        ...
        *render_area = new_area;
        {
            chunk_indices.retain(|&pos, &mut index| {
        ...
        for pos in new_area.iter() {
            if let Entry::Vacant(v) = chunk_indices.entry(pos) {
                if let Some(empty) = (0..is_used.len()).find(|&i| !is_used[i]) {
                    is_used[empty] = true;
                    v.insert(empty);
                    update_sender.send((empty, Some((pos, RenderChunkUpdate::new(&*world, pos))))).unwrap();
                } else {
                    let index = is_used.len();
                    is_used.push(false);
                    v.insert(index);
                    update_sender.send((index, Some((pos, RenderChunkUpdate::new(&*world, pos))))).unwrap();
                    return;
                }
            };
        }
